...
describe('Loading File - positive', () => {
  ...
  test('correctly loading in fixed output', async () => {
    ...
    const downloadedImage = await readFile(
      tmpDirPath,
      `${filesDir}/${imgName}`,
    );
    const downloadedScript = await readFile(
      tmpDirPath,
      `${filesDir}/${scriptName}`,
      'utf8',
    );
    const downloadedStyle = await readFile(
      tmpDirPath,
      `${filesDir}/${styleName}`,
      'utf8',
    );
    const downloadedCanonical = await readFile(
      tmpDirPath,
      `${filesDir}/${canonicalName}`,
      'utf8',
    );
...
describe('Loading File - negative', () => {
  test('bad request', async () => {
    ...
    await expect(
      pageLoader('http://my.url/not-exist-page', tmpDirPath),
    ).rejects.toThrow();
  });

  test('bad url', async () => {
    nock('http:/my.url').get(pagePath).reply(404, '');
    await expect(
      pageLoader('http:/my.url/not-exist-page', tmpDirPath),
    ).rejects.toThrow();
  });
  ...
  test('output path not exist', async () => {
    nock(baseUrl).get(pagePath).reply(200, 'data');
